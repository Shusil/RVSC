...
import sys
sys.path.append("gco_python-win")
...
#        if(np.sum(bpMask)>np.sum(bpVol[:,:,nearbySlice]) and idx<0):
#            ## Initial BP oversegmented, change BP threshold to threshGMM
...
#                unariesLLS = np.round(gBP.score_samples(vol.reshape(-1,1))).reshape(ROI.shape)
#                unariesLLT = np.round(gBG.score_samples(vol.reshape(-1,1))).reshape(ROI.shape)
...
#                eWeight1 = 5*itr*np.exp(-10*np.absolute(LL.ravel()[edges[:,0]]-LL.ravel()[edges[:,1]])/float(itr)).reshape(edges.shape[0],1)
                ...
                eWeight2 = 1000*np.exp(-10.0*(atlasGT.ravel()[edges[:,0]]+atlasGT.ravel()[edges[:,1]])).reshape(edges.shape[0],1)
