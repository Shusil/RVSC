# -*- coding: utf-8 -*-
"""
Created on Thu Jul 14 11:28:26 2016
...
#        decimalPlaces = 1
    ...
    #        for i in group: listedSliceSpacing.append(float(i[0].SpacingBetweenSlices))
            ...
            indxs = list(np.argsort(dist))
            indxCorrected = list(np.argsort(dist))
...
#                [uniqueSliceSpacings,unique_inverse,counts] = np.unique(roundSliceSpacings,return_inverse=True,return_counts=True)
            ...
            sliceSpacings = newDist[1:]-newDist[:-1]
    #        print(listedSliceSpacing)
            [uniqueSliceSpacings,unique_inverse,counts] = np.unique(np.round(sliceSpacings,0),return_inverse=True,return_counts=True)
            print(sliceSpacings)
            indx = np.argsort(counts)
#            actualSliceSpacing = uniqueSliceSpacings[indx[-1]]   # This is the correct slice spacing
        ...
        for slices in group:
            ...
            for phases in slices:
                ...
                if pattern in contourFiles:
                    img = 255*np.ones(phases.pixel_array.shape)
#                    data = []
...
#    exec("sio.savemat('N://ShusilDangi//RVSC//Test1SetMat//rawData"+str(ind+16)+".mat',rawPatientData)")
