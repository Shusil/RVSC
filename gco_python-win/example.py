...
def example_3D():
    ...
    # create potts pairwise
    pairwise = -10 * np.eye(2, dtype=np.int32)
#    pairwise = pairwise+np.array([[0,1],[1,0]])
#    # do simple cut
#    result = cut_simple(unaries, pairwise)
    ...
    horz = np.c_[inds[:,:-1,:].ravel(), inds[:,1:,:].ravel()]
    vert = np.c_[inds[:-1,:,:].ravel(), inds[1:,:,:].ravel()]
    depth = np.c_[inds[:,:,:-1].ravel(), inds[:,:,1:].ravel()]
    edges = np.vstack([horz, vert, depth]).astype(np.int32)
    eWeight = 10*np.exp(-np.absolute(x_noisy.ravel()[edges[:,0]]-x_noisy.ravel()[edges[:,1]])).reshape(edges.shape[0],1)
#    eWeight = np.absolute(x_noisy.ravel()[edges[:,0]]-x_noisy.ravel()[edges[:,1]]).reshape(edges.shape[0],1)
    edges = np.hstack((edges,eWeight)).astype(np.int32)    # we flatten the unaries
    result_graph = cut_from_graph(edges, unaries.reshape(-1, 2), pairwise)

    # plot results
    func.visualizeCompositeVol(x>0,np.array([7,1.5,1.5]))
    func.visualizeCompositeVol(x_thresh,np.array([7,1.5,1.5]))
    func.visualizeCompositeVol(result_graph.reshape(x.shape),np.array([7,1.5,1.5]))
